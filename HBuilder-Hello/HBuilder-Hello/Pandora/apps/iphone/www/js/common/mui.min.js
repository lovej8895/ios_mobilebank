...
(function($) {
	...
	//获取用户uuid插件调用
	$.plusReady(function(){
       var _BARCODE = 'pluginGetUDID', B = window.plus.bridge;
       var pluginGetUDID = {
	       getCustomUDID : function(){
	       return B.execSync(_BARCODE, "getCustomUDID");
	       }
       };
       window.plus.pluginGetUDID = pluginGetUDID;
       if($.os.ios){
       	 window.plus.device.uuid = pluginGetUDID.getCustomUDID()||plus.device.uuid;
       }
	});
...
(function($, window, document) {
    ...
    var createHubeiPopup = function(html, callback) {
        var popupElement = document.createElement('div');
        popupElement.className = CLASS_POPUP;
        popupElement.innerHTML = html;
        var removePopupElement = function() {
    ...
    var createHubeiAlert = function(message, title, btnValue, callback, type) {
        ...
        if (!$.os.plus || !$.os.ios || type === 'div') {
            return createHubeiPopup(createInner(message, title || '温馨提示') + createButtons([btnValue || '确定']), callback);
        }
        return plus.nativeUI.alert(message, callback, title || '温馨提示', btnValue || '确定');
    };
    //苹果用原生确认框，其他用重写确认框
    var createHubeiConfirm = function(message, title, btnArray, callback, type) {
        ...
        if (!$.os.plus || !$.os.ios || type === 'div') {
            return createHubeiPopup(createInner(message, title || '温馨提示') + createButtons(btnArray || ['取消', '确认']), callback);
        }
        return plus.nativeUI.confirm(message, callback, title || '温馨提示', btnArray || ['取消', '确认']);
    }; 
