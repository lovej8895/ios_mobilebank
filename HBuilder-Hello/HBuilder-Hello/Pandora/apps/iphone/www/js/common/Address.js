...
var addressInit = function(_cmbProvince, _cmbCity, _cmbArea, defaultProvince, defaultCity, defaultArea)
{
	...
	cmbProvince.addEventListener('click',function(){
		var province = {};
		var bts = [];
		for(var index=0; index<provinceList.length; index++){
			province = {title:provinceList[index].name};			
			bts.push(province);
		}
		plus.nativeUI.actionSheet({title:"省份选择",cancel:"取消",buttons:bts},
			function(e){
				if(e.index>0){
					cmbProvince.innerText = bts[e.index-1].title;
					currProvince = cmbProvince.innerText;
					currProIndex = e.index-1;
					if(32 > e.index){
						cmbCity.innerText = provinceList[e.index-1].cityList[0].name;
						cmbArea.innerText = "";
					}else{
						cmbCity.innerText = "";
						cmbArea.innerText = "";
	...
	cmbCity.addEventListener('click',function(){
		var city = {};
		var bts = [];	
		if(null == currProvince || "" == currProvince ){
			mui.alert("请先选择省份");
			return;
		}else if(32<currProIndex){
			return;
		}
		for(var item=0; item<provinceList[currProIndex].cityList.length; item++){
			city = {title:provinceList[currProIndex].cityList[item].name};
			bts.push(city);
		}		
		plus.nativeUI.actionSheet({title:"城市选择",cancel:"取消",buttons:bts},
				function(e){
					if(e.index>0){
						cmbCity.innerText = bts[e.index-1].title;
						currCity = cmbProvince.innerText;
						currCityIndex = e.index-1;
						cmbArea.innerText = provinceList[currProIndex].cityList[currCityIndex].areaList[0];
					}					
				}
		);		
	});
	
	cmbArea.addEventListener('click',function(){
		var areaCode = {};
		var bts = [];	
		if( 32<currProIndex ){
			return;
		}
		for(index=0; index<provinceList[currProIndex].cityList[currCityIndex].areaList.length;index++){
			areaCode = {title:provinceList[currProIndex].cityList[currCityIndex].areaList[index]};
			bts.push(areaCode);
		}
		plus.nativeUI.actionSheet({title:"城市选择",cancel:"取消",buttons:bts},
				function(e){
					if(e.index>0){
						cmbArea.innerText = bts[e.index-1].title;
					}					
				}
		);	
	});
