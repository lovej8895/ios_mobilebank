...
(function($, window, document) {
    ...
    //苹果用原生弹出框，其他用重写的弹出框
    var createHubeiAlert = function(message, title, btnValue, callback, type) {
        if (typeof message === 'undefined') {
            return;
        } else {
            if (typeof title === 'function') {
        ...
        if (!$.os.plus || !$.os.ios || type === 'div') {
            return createPopup(createInner(message, title || '提示') + createButtons([btnValue || '确定']), callback);
        }
        return plus.nativeUI.alert(message, callback, title || '提示', btnValue || '确定');
    };
    //苹果用原生确认框，其他用重写确认框
    var createHubeiConfirm = function(message, title, btnArray, callback, type) {
        if (typeof message === 'undefined') {
            return;
        } else {
            if (typeof title === 'function') {
...
//  $.alert = createAlert;
    $.alert = createHubeiAlert;
//  $.confirm = createConfirm;
    $.confirm = createHubeiConfirm;
    $.prompt = createPrompt;
})(mui, window, document);
(function($, document) {
