define(function(require, exports, module) {
    ...
    var accountPicker;
    var transferRadio=null;//交易类型单选按钮值
    var timeList = [];
    var transferList = [];
	mui.init();
	mui.plusReady(function() {		
		...
		function queryDefaultAcct(){
			...
			transferList = jQuery.param.getParams('TRANSFER_LIST');	//得到交易类型		
			$("#transferType").html(transferList[0].text);
			transferTypeInit();
			function transferTypeInit(){
				var transferPicker = new mui.SmartPicker({title:"请选择交易类型",fireEvent:"transferType"});
			...
			window.addEventListener("transferType",function(event){
	            ...
	            transferRadio = param.value;
				$("#transferType").html(param.text);
	        });	
			...
			if(day<10){
				...
				if(f_prodcode!=""){
					...
					var params = {
							...
							"f_end_date":endDate,
							"f_businesscode":transferRadio,
							noCheck:noCheck
			};	
			mbank.openWindowByLoad("../fund/historyTranQueryResult.html", "historyTranQueryResult", "slide-in-right", params);
