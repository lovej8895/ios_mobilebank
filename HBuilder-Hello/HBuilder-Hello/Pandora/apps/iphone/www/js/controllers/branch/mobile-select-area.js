...
//	selectArea.init({ trigger: '#cityName_a', value: $('#cityName_a').data('value'),
//		text: ['湖北省', '武汉市', '江夏区'],data: dataJson, eventName: 'click', position: "bottom" });
...
//			this.level = level > 0 ? level : 3;
		...
		show:function(){
			...
			var dlgContent = '';
				...
				$.confirm('<div class="ui-scroller-mask"><div id="' + _this.id + '" class="ui-scroller">'
				+ dlgContent + '<p></p></div></div>', buttons, function(t, c) {
					if (t == "yes") {
						_this.submit()
					}
					if (t == 'no') {
...
//			console.log(_this.text)
		...
		submit: function() {
			this.oldvalue = this.value.concat([]);
			this.oldtext = this.text.concat([]);
			console.log("标签类型："+this.trigger[0].nodeType);
			if (this.trigger[0].nodeType == 1) {
				//input
//				var strs = new Array();//定义数组
//				var strsVal = new Array();//定义数组
////				strs = (this.text + '').split(',');//字符分割 注：解决this.text.split is not a function 异常 字符后面需加上空字符
//					strs = this.text.join(',').split(',');//字符分割
//				var str = strs[1];
//					console.log("this.text.join(','):"+this.text.join(','));
//					console.log("str:"+str);
//					strsVal = this.value.join(',').split(",");
//				var strVal = strsVal[1];
//					console.log("this.value.join(','):"+this.value.join(','));
