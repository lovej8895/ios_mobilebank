define(function(require, exports, module) {
	...
	mui.plusReady(function(){
		...
		$("#nextStep").on("tap",function(){
        	...
        	mbank.openWindowByLoad('thisAppointRefund_Signing_Input.html','thisAppointRefund_Signing_Input','slide-in-right',params);
        });
        ...
        $("#nextStep2").on("tap",function(){
        	...
        	mui.confirm("您是否确认解除约定还款账户？解约之后，我行将不再发起约定扣款，请您自助还款，以免造成逾期。","温馨提示",["确认", "取消"], function(event){
				if (event.index == 0) {
		    		...
		    		mbank.apiSend("post",url,params1,successCallback,errorCallback,true);
		    		function successCallback(data){
		        	mbank.openWindowByLoad('../creditCard/creditCardPublic_Result.html','creditCardPublic_Result','slide-in-right',params1);
		    		}
        ...
        $("#nextStep3").on("tap",function(){
        	...
        	mbank.openWindowByLoad('thisAppointRefund_Signing_Input.html','thisAppointRefund_Signing_Input','slide-in-right',params2);
        });
	});
});