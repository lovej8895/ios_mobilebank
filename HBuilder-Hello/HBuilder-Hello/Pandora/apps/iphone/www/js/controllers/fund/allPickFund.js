define(function(require, exports, module) {
    ...
    mui.plusReady(function() {
		...
		var self=plus.webview.getWebviewById("treasureClassTransQueryDetail");  //从父页面中得到前一个页面传过来的值
			...
			function transferSuccess(data){
				...
				if(f_treasureTransDetailQuery.length>0){
					for(var i=0;i<data.f_treasureTransDetailQuery.length;i++){
						...
						if(singleList.f_businesscode==22){ //转入输出绿字
							html += '<span class="detail_right"><span class="color_g">'+format.formatMoney(singleList.f_confirmedamount,2)+'</span>元</span></p>';
						}else if(singleList.f_businesscode==98){ //转出输出红字
							html += '<span class="detail_right"><span class="color_red">'+format.formatMoney(singleList.f_confirmedamount,2)+'</span>元</span></p>';	
						}
						...
						if(singleList.f_businesscode==22){
						...
						}else if(singleList.f_businesscode==98){
							html += '<p class="fund_detail_amt"><span class="color_red fz_16">'+format.formatMoney(singleList.f_confirmedamount,2)+'</span><span >元</span></p>';
						}else if(singleList.f_businesscode==24){
							html += '<p class="fund_detail_amt"><span class="color_red fz_16">'+format.formatMoney(singleList.f_confirmedamount,2)+'</span><span >元</span></p>';
						}
						html +='</li>';
					}
		...
		document.getElementById("submit").addEventListener('tap',function(){
			...
			plus.webview.close(plus.webview.getWebviewById("allPickFund"));
		});
		//增加右滑
		window.addEventListener("swipeleft",function(e){
	    	if (Math.abs(e.detail.angle) > 170) {
		    	plus.webview.getWebviewById("allPickFund").hide();
		    	plus.webview.getWebviewById("transIntoFund").show();
		    	plus.webview.getWebviewById("transOutFund").hide();
		    	mui.fire(self,"changeMenu",{pageId:"transIntoFund"});	    		
	    	}
	    });    
	});
