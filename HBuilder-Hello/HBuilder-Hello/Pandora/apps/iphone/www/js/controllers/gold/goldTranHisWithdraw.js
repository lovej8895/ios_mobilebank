...
//			turnPageBeginPos = turnPageBeginPos + turnPageShowNum;
	...
	mui.plusReady(function() {
				...
				if(data.ec == "000"){
					...
					if(queryList.length>0){
						for(var i = 0; i < queryList.length; i++){
							...
							html +='<p class="color_6">'+jQuery.param.getDisplay("GOLD_ORDER_TYPE",queryList[i].tranType)+'</p>';
							...
							html +='<p class="fz_14 color_g m_top5px">'+jQuery.param.getDisplay("GOLD_CHARGE_STATE",queryList[i].orderState)+'</p>';
							html +='</div>';
        ...
        mui('#queryList').on('tap','li',function(event) {
			...
			mbank.openWindowByLoad("../gold/goldTranHisWithdrawDetail.html","goldTranHisWithdrawDetail", "slide-in-right",params);
		});
