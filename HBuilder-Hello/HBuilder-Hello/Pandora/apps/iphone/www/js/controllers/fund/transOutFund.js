define(function(require, exports, module) {
    ...
    mui.plusReady(function() {
				...
				if(f_treasureTransDetailQuery.length>0){
					for(var i=0;i<data.f_treasureTransDetailQuery.length;i++){
						var singleList = data.f_treasureTransDetailQuery[i];
							...
							html += '<span class="detail_right"><a class="color_red">'+format.formatMoney(singleList.f_confirmedamount,2)+'</a>元</span></p>';	
		...
		document.getElementById("submit").addEventListener('tap',function(){
			...
			plus.webview.close(plus.webview.getWebviewById("transOutFund"));
		});
		//增加左滑
