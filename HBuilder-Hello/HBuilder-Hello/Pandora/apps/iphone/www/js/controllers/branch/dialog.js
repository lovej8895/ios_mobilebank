...
//			html += '<p class="ui-dialog-action"><span class="ui-alert-submit  js-dialog-close">请选择省份和城市</span></p>';
			var action = '';
		...
		if (settings.type === "confirm") {
			...
			console.log("settings.content=="+settings.content);
			var action = '';
			if (!settings.buttons) {
				settings.buttons = [{
					'yes': '确定'
			...
			for (var i = 0, l = settings.buttons.length; i < l; i++) {
				...
				if (item.title) {
					btnstr += '<td><button class="ui-confirm-close js-dialog-close" data-type="close">' + item.title + '</button></td>';
				}
			}
			action = '<table class="ui-dialog-action"><tr>' + btnstr + '</tr></table>';
//			action = '<table class="ui-dialog-action"><tr>' + btnstr + '</tr><tr><td><span>省份</span></td><td><span>城市</span></td></tr></table>';
			...
			var options = $.extend({
				...
				beforeShow: function(c) {
					...
					dialog.touch($('.ui-confirm-close', c), function() {
						settings.callback && settings.callback.call(dialog, 'title', c);
					});
				}
			}, settings);
			dialog.init(options);
